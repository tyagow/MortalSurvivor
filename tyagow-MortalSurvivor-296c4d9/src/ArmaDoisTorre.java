...
public class ArmaDoisTorre extends Arma {
	...
	public void DesenhaSe(Graphics2D dbg, int XMundo, int YMundo) {
		...
		if (estado==1) {
			...
			dbg.fillRect(GamePanel.PWIDTH/2-50, GamePanel.PHEIGHT/2-205,(int)(getTempoRecarrega()*100/Constantes.TORRE_ARMA_DOIS_tempoRecarrega) , 20);
	...
	private void calculaIA(int DiffTime) {
		...
		if (estado==0) {//atirando
			...
			if (getTempoEntreTiros()>=Constantes.TORRE_ARMA_DOIS_tempoEntreTiros) {
				if (atirou) {	
					atira();
					setTempoEntreTiros(0);
		...
		if (estado==1) {
			...
			if (getTempoRecarrega()>=Constantes.TORRE_ARMA_DOIS_tempoRecarrega) {
